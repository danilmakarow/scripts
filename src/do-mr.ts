#!/usr/bin/env node
/**
 * domr — stage, commit, push, and open a merge request / pull request.
 *
 * Mirrors `docheckout`'s behaviour (verify repo + changes, detect master,
 * stage, ask Claude Haiku for a commit message, branch off master if needed,
 * push) and then layers the MR/PR step on top:
 *
 *   1. After push, query the host (GitLab or GitHub) for an open MR/PR
 *      sourced from the current branch.
 *   2. If one exists, print its URL and exit (idempotent re-run).
 *   3. Otherwise create a new MR/PR targeting the repo's default branch with
 *      the commit subject as title and commit body as description.
 *
 * Usage:
 *   domr
 *
 ...
 *   GIT_ACCESS_TOKEN    — required, used to talk to the GitLab/GitHub API.
 */
...
import { GitClient, GitClientError, type GitHost } from './common/git-client';
...
interface AppEnv {
  ...
  readonly GIT_ACCESS_TOKEN: string;
}
...
// ─────────────────────────────────────────────────────────────
// Prompt builders (kept in sync with do-checkout for consistent output)
// ─────────────────────────────────────────────────────────────
/** Conventional Commits system prompt shared with do-commit / do-checkout. */
...
/** Defensive JSON.parse that tolerates stray prose around the JSON object. */
 ...
 * suggestion. When `includeBranchName` is true the response must include a
 * kebab-case `<type>/<slug>` branch name.
 */
...
const writeCommitMessageFile = (message: string): string => {
  const filePath = path.join(
    os.tmpdir(),
    `domr-commit-${process.pid}-${Date.now()}.txt`,
  );
...
/**
 * Splits a commit message into `{ subject, body }`. The body is everything
 * after the first blank line, trimmed; or empty when no body is present.
 */
const splitCommitMessage = (message: string): { readonly subject: string; readonly body: string } => {
  const trimmed = message.trim();
  const lines = trimmed.split('\n');
  const subject = lines[0]?.trim() ?? trimmed;

  // Skip the first blank line if present, then take the rest as body.
...
// Usage banner
// ─────────────────────────────────────────────────────────────
/** Prints the domr usage banner. */
const showUsage = (): void => {
  printUsage({
    title: 'domr',
    description: 'Stage, commit, push, and open a merge request / pull request',
    usage: 'domr',
    examples: [{ command: 'domr' }],
    steps: [
      ...
      'Find an existing open MR/PR or create a new one targeting the default branch',
    ],
  });
};
